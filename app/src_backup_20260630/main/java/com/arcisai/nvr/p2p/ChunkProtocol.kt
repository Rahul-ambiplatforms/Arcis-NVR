...
object ChunkProtocol {
    ...
    // 1100 bytes = 1084-byte payload + 16-byte header.
    // The provider's internal bitmap maps chunk_index = offset / 1084, so only a
    // 1084-byte payload keeps every chunk at a unique index.  Sending 484-byte
    // chunks causes index collisions (offset 0 and 484 both land in slot 0) and
    // the provider never reassembles the frame.  1100-byte packets are well within
    // the 1500-byte Ethernet MTU even after TURN and IP headers (~1140 bytes on
    // the wire), so they are not fragmented or dropped on any normal path.
    const val MAX_UDP_PAYLOAD = 1100
    const val HEADER_SIZE     = 16
    const val MAX_CHUNK       = MAX_UDP_PAYLOAD - HEADER_SIZE   // 1084
}
