...
class RemoteSession(
    ...
    // Called from the AgentListener when ICE transitions to FAILED after we were
    // already connected.  Zeroing lastPongAt makes isAlive return false immediately
    // so the 5-second liveness poller triggers a reconnect on the next tick rather
    // than waiting out the full 30-second PONG death timeout.
    private fun forceDeadNow() { lastPongAt = 0L }
            ...
            override fun onStateChanged(state: Int) {
                ...
                when (state) {
                    ...
                    5    -> if (!stateConnected.isCompleted) stateConnected.complete(false)
                                 ...
                                 // ICE FAILED after we were already connected — mark the
                                 // session dead immediately so the 5-second poller triggers
                                 // a reconnect on the next tick instead of waiting 30 s.
                                 Log.w(tag, "[$serviceId] ICE FAILED post-connect — forcing dead")
                                 forceDeadNow()
                             }
                }
            }
