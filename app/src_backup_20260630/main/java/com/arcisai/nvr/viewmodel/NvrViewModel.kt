...
class NvrViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private suspend fun attemptReconnectMain(creds: NvrCredentials) {
        ...
        try {
            ...
            for (attempt in 1..3) {
                remoteStatus = if (attempt == 1) "Reconnecting…" else "Reconnecting… ($attempt/3)"
                val ns = RemoteSession(creds.deviceId, remoteConfig)
                if (!ns.connect()) {
                    ns.close()
                    android.util.Log.w("NvrViewModel", "attemptReconnectMain: connect() failed (attempt $attempt/3)")
                } else {
                    ...
                    android.util.Log.i("NvrViewModel", "attemptReconnectMain: ok, new port=${ns.localPort} (attempt $attempt/3)")
                    nvrOnline = true
                    ...
                    // the moment streamRefreshToken fires.
            ...
            if (!reconnected) {
                remoteStatus = null
                android.util.Log.w("NvrViewModel", "attemptReconnectMain: all 3 attempts failed")
            }
        } catch (t: Throwable) {
